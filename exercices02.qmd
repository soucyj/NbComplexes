...
d. Forme cartésienne: $4$,
   ...
   Forme polaire: $10\left(\cos\left(0\right)+\iu\sin\left(0\right)\right)$.
...
f. Forme cartésienne: $-4$,
   ...
   Forme polaire: $3\left(\cos\left(\piu\right)+\iu\sin\left(\piu\right)\right)$.
...
h. Forme cartésienne: $-3+\iu\sqrt{3}$,

   Module: $2\sqrt{3}$,
   
   Argument principal: $\frac{5\piu}{6}$,
   
   Forme polaire: $2\sqrt{3}\left(\cos\left(\frac{5\piu}{6}\right)+\iu\sin\left(\frac{5\piu}{6}\right)\right)$.
...
::: {.callout-tip collapse="true"}
## Solution
a. Nous avons que $1-\iu=\sqrt{2}\left(\cos\left(-\frac{\piu}{4}\right)+\iu\cos\left(-\frac{\piu}{4}\right)\right)$. Ainsi, le module du nombre $(1-\iu)^{12}$ sera $(\sqrt{2})^{12}=2^6$, et un de ses arguments est $-\frac{\piu}{4}\cdot 12=-3\piu$. L'argument principal est donc $\piu$. Cela nous permet finalement d'écrire
...
d. Le nombre $1-\iu\sqrt{3}$ a pour module 2 et pour argument principal $-\frac{\piu}{3}$. Ainsi, $(1-\iu\sqrt{3})^{10}$ a pour module $2^{10}$ et $\frac{2\piu}{3}$ comme argument principal. De même, le nombre $9i$ a pour module 9 et pour argument principal $\frac{\piu}{2}$. Le produit de ces deux nombres est donc le nombre complexe ayant pour module $2^{10}\cdot 9 = 9216$, et pour argument $\frac{\piu}{2}+\frac{2\piu}{3}=\frac{7\piu}{6}\equiv -\frac{5\piu}{6}~\pmod{2\piu}$.
Ainsi,
$$(1-\iu\sqrt{3})^{10}\cdot 9i=\underbrace{9216\left(\cos\left(-\frac{5\piu}{6}\right)+\iu\sin\left(-\frac{5\piu}{6}\right)\right)}_{\text{forme~polaire}}=\underbrace{-4608\sqrt{3}-4608\iu~.}_{\text{forme cartsienne}}$$
:::
...
::: {.callout-tip collapse="true"}
## Solution
Chuck et Norris ont tous les deux raison. En effet, en considérant l'identité $$\cos(2\theta)=\cos^2(\theta)-\sin^2(\theta)$$ qui a été démontrée à l'exercice @exr-DeMoivre, de même que l'identité fondamentale $$\cos^2(\theta)+\sin^2(\theta)=1,$$ on obtient que $\cos^2(\theta)=1-\sin^2(\theta)$ et $\sin^2(\theta)=1-\cos^2(\theta)$. En substituant l'expression pour $\cos^2(\theta)$ dans l'équation $\cos(2\theta)=\cos^2(\theta)-\sin^2(\theta)$ on obtient la version de Norris. En substituant l'expression pour $\sin^2(\theta),$ on obtient celle de Chuck.
:::
...
::: {#exr-sin13pisur12}
\n
En vous servant de ce qui a été fait au numéro à l'@exr-cospisur12, déterminez la valeur exacte de $\sin\left(\frac{13\piu}{12}\right)$
:::
...
$$\sin^2\left(\frac{\piu}{12}\right)=1-\cos^2\left(\frac{\piu}{12}\right)=\frac{\sqrt{2-\sqrt{3}}}{4}.$$
Comme $\sin\left(\frac{\piu}{12}\right)>0$, il suit que $\sin\left(\frac{\piu}{12}\right)=\frac{\sqrt{2-\sqrt{3}}}{4}.$ De cela nous pouvons conclure que
$\sin\left(\frac{13\piu}{12}\right)=-\frac{\sqrt{2-\sqrt{3}}}{4}$.
:::
...
\end{eqnarray*}
d. Le nombre $0$ n'a qu'une seule racine carrée, à savoir $0$.\
...
Pour $n=3$, les trois arguments sont $\frac{0+2k\piu}{n}$ où $k\in\{0,1,2\}$, ce qui nous donne les arguments $0, \frac{2\piu}{3}$ et $\frac{4\piu}{3}$. Ainsi,
\begin{align*}
...
Pour $n=4$, les quatre arguments sont $\frac{0+2k\piu}{n}$ où $k\in\{0,1,2,3\}$, ce qui nous donne les arguments $0, \frac{\piu}{2},\piu$ et $\frac{3\piu}{2}$. Ainsi,
\begin{align*}
	z_0&=1\cdot(\cos 0 +\iu \sin 0)=1,\\
...
Pour $n=5$, les cinq arguments sont $\frac{0+2k\piu}{n}$ où $k\in\{0,1,2,3,4\}$, ce qui nous donne les arguments $0, \frac{2\piu}{5},\frac{4\piu}{5},\frac{6\piu}{5}$, et $\frac{8\piu}{5}$. Ainsi,
\begin{align*}
	z_0&=1\cdot(\cos 0 +\iu \sin 0)=1,\\
...
Pour $n=6$, les six arguments sont $\frac{0+2k\piu}{n}$ où $k\in\{0,1,2,3,4,5\}$, ce qui nous donne les arguments $0,\frac{\piu}{3},\frac{2\piu}{3},\piu,\frac{4\piu}{3}$, et $\frac{5\piu}{3}$. Ainsi,
\begin{align*}
	z_0&=1\cdot(\cos 0 +\iu \sin 0)=1,\\
